...
- `components/inventory/product-table.tsx` — shared table. Product name links to its detail page. `current_stock` has its own always-visible quick-update form (input + 更新); `unit`/`low_stock_threshold`/`storage_location`/`notes` are each a click-to-edit `InlineEditableField` cell (click → input + 保存/キャンセル, submits via the generic `updateProductField` action, remounts closed on success by being keyed on the field's own value). A row-level delete button (`ConfirmDeleteButton` with a `Trash2` icon, `deleteProduct` with no `redirect_to` so it stays on the current page) sits next to 更新.
